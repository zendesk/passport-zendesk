...
function zendeskURL(subdomain, path) {
  var baseUrl = new URL(`https://${subdomain}.zendesk.com`);

  if (!baseUrl.host.endsWith('.zendesk.com')) {
...
function authorizationURL(subdomain) {
  return zendeskURL(subdomain, '/oauth/authorizations/new');
}

function tokenURL(subdomain) {
  return zendeskURL(subdomain,'/oauth/tokens');
}

function profileURL(subdomain) {
  return zendeskURL(subdomain, '/api/v2/users/me.json');
}
...
function Strategy(options, verify) {
  var subdomain = options.subdomain || 'www';
  options = options || {};
  options.scopeSeparator = options.scopeSeparator || ' ';
  options.customHeaders = options.customHeaders || {};
  options.scope = options.scope || ['read'];
  options.sessionKey = options.sessionKey || 'oauth2:zendesk.com';
  options.authorizationURL = authorizationURL(subdomain);
  options.tokenURL = tokenURL(subdomain);
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if (!subdomain) {
    return this.error(new Error('A Zendesk subdomain was not specified in options and was not found in request parameters'));
  }
...
Strategy.prototype.userProfile = function(accessToken, subdomain, done) {
  ...
  oauth2.get(profileURL(subdomain), accessToken, function(err, body) {
    var json;
    var profile = {};
